...
construir_variables_ocu <- function(datos){
  ...
  datos_procesados <- datos |> 
    ...
    mutate(
      ...
      division = if_else(p3 == "9999", "99", division),
      # ingreso mensual
      ...
      # tasa crítica de ocupación (INEGI, 2023: 30)
...
# tasa de variación
calcular_tasa_var <- function(datos, nivel, nombre){
...
calcular_descriptivos <- function(datos, variable){
  ...
  datos |> 
    ...
    summarise(
      ...
      variable = nombre_var, 
      minimo = min({{  variable  }}),
      promedio = survey_mean({{  variable  }}, vartype = c("se", "cv", "ci")),
...
calcular_descriptivos_map <- function(datos, variable){
  ...
  poblacion |> 
    ...
    summarise(
      ...
      media = survey_mean(  !!variable_sym  , vartype = c("se", "cv", "ci")),
...
validar_precision <- function(datos, estimador_cv){
  ...
  mutate(
    nivel_precision = case_when(
      {{  estimador_cv  }} <= 0.15 ~ "Alta",
      {{  estimador_cv  }} > 0.15 & {{  estimador_cv  }} <= 0.30 ~ "Moderada",
      {{  estimador_cv  }} > 0.30 ~ "Baja",
      TRUE ~ NA
    )
  )
}
