...
```{r}
# levels(objetivo$ocupaciones)
# levels(objetivo$carreras)
# levels(objetivo$estados)
# levels(objetivo$pos_ocu)
# levels(objetivo$sector_eco)
# levels(objetivo$trabajadores)
# levels(objetivo$tamanio)
```

Todo quedó correctamente definido, excepto el factor `estados`. Para dicha variable, existen dos soluciones:

1. Definir los niveles permitidos
2. Agruparlos por regiones

En breve, se definirás las regiones. Según <https://portalhcd.diputados.gob.mx/PortalWeb/Micrositios/69e1e1bf-d716-46ca-932f-9c6ab78068c6.pdf>, las regiones se dividen como sigue:

* sur: Guerrero, Oaxaca, Chiapas, Veracruz, Tabasco, Campeche, Yucatán y Quintana Roo, en la ENOE: 12, 20, 7, 30, 27, 4, 31, 23
* centro: Guanajuato, Querétaro, Hidalgo, México, Ciudad de México, Morelos, Tlaxcala y Puebla: 11, 22, 13, 15, 9, 17, 29, 21
* centro-norte: Jalisco, Aguascalientes, Colima, Michoacán y San Luis Potosí: 14, 1, 6, 16, 24
* norte: Baja California, Sonora, Chihuahua, Coahuila, Nuevo León y Tamaulipas: 2, 26, 8, 5, 19, 28
* norte-occidente: Baja California Sur, Sinaloa, Nayarit, Durango y Zacatecas: 3, 25, 18, 10, 32

Se crea la variable `regiones`
...
Se validan los niveles

```{r}
levels(objetivo$regiones)
```

Por último, se realiza la regresión
...
```{r}
modelo <- lm(log(ingreso_mensual) ~ anios_estudios + over_obj + exper + I(exper**2) + log(horas_lab) + mujer + casada + jefe_hogar + ocupaciones + regiones, data = reg_general)
...
```{r}
modelo <- lm(log(ingreso_mensual) ~ anios_estudios + over_obj + exper + I(exper**2) + log(horas_lab) + mujer + casada + jefe_hogar + regiones + escolaridad + carreras + pos_ocu + sector_eco + trabajadores + tamanio + informal, data = reg_jovenes)

summary.lm(modelo)
```

Se observa que se eliminan muchas observaciones, ¿por qué? porque en las variables dicotómicas hay valores NA

Términos de interacción

```{r}
modelo_int <- lm(log(ingreso_mensual) ~ anios_estudios + over_obj + exper + I(exper**2) + log(horas_lab) + mujer + casada + jefe_hogar + regiones + escolaridad + carreras + pos_ocu + sector_eco + trabajadores + tamanio + informal + over_obj:mujer + over_obj:regiones + over_obj:escolaridad + over_obj:carreras + over_obj:sector_eco + over_obj:tamanio + over_obj:informal, data = reg_jovenes)

summary.lm(modelo_int)
```

Se eliminan términos no significativos

```{r}
modelo_int <- lm(log(ingreso_mensual) ~ anios_estudios + over_obj + exper + I(exper**2) + log(horas_lab) + mujer + casada + jefe_hogar + regiones + escolaridad + carreras + informal + over_obj:mujer + over_obj:regiones + over_obj:escolaridad + over_obj:carreras + over_obj:informal, data = reg_jovenes)

summary.lm(modelo_int)
```

Por último, se elimina o no se considera la variable `trabajadores` considero que brinda la misma información que `tamanio`
