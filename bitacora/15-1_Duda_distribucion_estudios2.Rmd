...
A diferencia de "C:/Prog/tesis/11_Duda_distribucion_estudios.Rmd", aquí se utiliza el ponderador trimestral `fac_tri` y no el tamaño de la muestra.
...
ocupada %>% 
  ...
  mutate(
    prop = tot / sum(tot)
    # prop = percent(prop, accuracy = 3)
  ) 
```

Al igual que cuando se considera el tamaño de la muestra, las ocupaciones 9 y 2 concentran casi el 40% del total de ocupaciones (20% y 18%, respectivamente). Por su parte, las ocupaciones que requieren estudios universitarios concentran el 22%.
...
ocupada %>% 
  ...
  mutate(
    prop = tot / sum(tot)
    # prop = percent(prop, accuracy = 2)
  )
  # arrange(desc(prop))
```

En este caso, el 42% de las personas ocupadas reportan estudios de nivel básico (16% primaria y 28% secundaria). En el caso de trabajadores con estudios de nivel superior (desde el valor 5 al 8), concentran el 28%.

Al comparar, la distribución entre ocupaciones y trabajadores se observa un déficit de demanda de 6%: 22% ocupaciones y 28% trabajadores. 
...
x2 %>% left_join(x1, by = c("trabajadores" = "ocupaciones")) %>% 
  mutate(
    dif = ocup - trab,
    desajuste = if_else(ocup < trab, "deficit", "superavit")
  ) %>% 
  rename("escolaridad" = "trabajadores")
```

Se observa que en el caso de ocupaciones que requieren estudios de nivel superior, existe un déficit de demanda alrededor del 8%, mientras que en ocupaciones con estudios inferiores existe un superávit. Estos resultados demuestran que el problema del déficit de demanda para el caso de profesionistas persiste en pleno 2025, recordemos que los estudios revisados en el capítulo anterior demuestran la existencia del problema desde la década de los noventa, es decir, podríamos decir que éste problema se encuentra presente por lo menos desde hace treinta años en México.
